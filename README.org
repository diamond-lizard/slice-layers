...
*** Alternatives
**** [[https://gmic.eu/][G'MIC]]'s [[https://twitter.com/gmic_eu/status/1234525005725229056][Sequences / Moiré Animation filter]]
This filter has a similar effect, but:
***** it does not allow one to set the slice width
***** it only generates a merged frame and a mask
so it's impossible to manipulate the original layers after applying the filter to get a different effect
** Installation
*** 1
